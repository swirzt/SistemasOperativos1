...
tateti(Tablero, Plays, Turno, Juegoid) ->
    receive
      {Cartero, local, bye} ->
          {Jugada, Jugador} = {abandon, w2};
      {Cartero, away, bye} ->
          {Jugada, Jugador} = {abandon, w1};
      {Cartero, Cond, Pos} ->
          {Jugada, Jugador} = {Pos, Cond}
    end,
    if (Jugada == 0) and (Jugador == local) ->
           Cartero ! {w2, Tablero},
           funcionMataJuego(Juegoid);
       (Jugada == 0) and (Jugador == away) ->
           Cartero ! {w1, Tablero},
           funcionMataJuego(Juegoid);
       Jugada == abandon ->
           Cartero ! {Jugador, Tablero},
           funcionMataJuego(Juegoid);
       (Jugador == local) and (Turno == -1) ->
           Cartero ! error,
           tateti(Tablero, Plays, Turno, Juegoid);
       (Jugador == away) and (Turno == 1) ->
           Cartero ! error,
           tateti(Tablero, Plays, Turno, Juegoid);
       true ->
           case jugada(Tablero, Jugada, Turno) of
             {ok, TableroN} ->
                 K = Plays + 1,
                 case checkGanador(TableroN) of
                   ...
                   nada ->
                       if K == 9 ->
                              Cartero ! {empate, TableroN},
                              funcionMataJuego(Juegoid);
                          true ->
                              Cartero ! {tablero, TableroN},
                              tateti(TableroN, K, -Turno, Juegoid)
                       end
                 end;
             error ->
                 Cartero ! error,
...
funcionMataJuego(Juegoid) ->
    case obtieneNodo(Juegoid) of
      {ok, NodoJuego} ->
          ...
          receive
            {Local, Visitante, _, _} ->
                {ok, Nodo1} = obtieneNodo(Local),
                {usuarios, Nodo1} ! {borrarJuego, Local, Juegoid},
                case Visitante of
                  nadie ->
                      ok;
                  _ ->
                      {ok, Nodo2} = obtieneNodo(Visitante),
      ...
      {error, _} ->
          ok %% No deberÃ­a entrar a esta zona porque siempre se llama con un JuegoId correcto
    end.
