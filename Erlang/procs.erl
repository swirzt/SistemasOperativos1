...
work(Pid) ->
    receive
        fin -> ok;
        {0, _} -> Pid ! fin;
        {N, Msj} -> io:format("Recibí ~p, quedan ~p ~n",[Msj,N-1]),
                    Pid ! {N-1, Msj},
                    work(Pid);
        {inic,N,Msj} -> io:format("Comienzo a mandar '~p' ~p veces ~n",[Msj,N]),
                        Pid ! {N, Msj},
                        work(Pid)
    end.
...
working(Pid) ->
    receive
        ...
        {inic,Msj,M,N} -> io:format("Comienzo a mandar '~p' ~p veces ~n",[Msj,M]),
                          Pid ! {Msj, M, N},
                          working(Pid);
        % Termino de enciar mensajes, empiezo a matar procesos
        {_,0,N} -> io:format("Terminando procesos ~n"),
                     Pid ! {fin, N-2}; %Es N-2 porque hay que mandar N-1 fin totales
                                       %Y este ya es uno
        % Recibo el mensaje, lo reenvio con M-1
        {Msj,M,N} -> io:format("Recibí ~p, quedan ~p ~n",[Msj,M-1]),
                     Pid ! {Msj, M-1, N},
                     working(Pid);
        % Es el ultimo, no debe hacer nada y termina
        {fin,0} -> io:format("Mori~n"),ok;
...
initializar(0, Ps) -> Ps;
initializar(N, Ps) -> X = spawn(?MODULE,working,[inic]),
...
conectar(PUltimo, PActual, [P|Ps]) -> PActual ! P,
...
startAnillo(M,N,Msj) -> List = initializar(N,[]),
                        X = head(List),
